...
async function collectQualityBaseline() {
  try {
    ...
    const nullCount = allEvents.filter(e => !e.official_url).length;
    ...
    allEvents.forEach(event => {
      if (event.official_url && aggregators.some(agg => event.official_url.includes(agg))) {
        aggregatorCount++;
      }
    });
    ...
    // 4. カテゴリ別イベント件数（race_type で集計）
    const categoryCount = {};
    allEvents.forEach(event => {
      const cat = event.race_type || 'unknown';
      categoryCount[cat] = (categoryCount[cat] || 0) + 1;
    });
    
    console.log(`\n4. カテゴリ別イベント件数（race_type）:`);
    ...
    const enrichStatus = {};
    allEvents.forEach(event => {
      const status = event.enrich_quality || 'null';
      enrichStatus[status] = (enrichStatus[status] || 0) + 1;
    });
    
    console.log(`\n5. enrich 関連ステータスの分布（enrich_quality）:`);
