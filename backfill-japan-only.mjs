/**
 * 日本イベント NULL座標 のみ再ジオコーディング（高速版）
 *
 * 使い方:
 *   node backfill-japan-only.mjs --dry-run    # 確認のみ
 *   node backfill-japan-only.mjs              # 実行
 */
...
async function main() {
  ...
  console.log(`=== 日本イベント NULL座標 backfill 開始 (DRY_RUN: ${DRY_RUN}) ===\n`)

  // 対象レコードを取得: location に「日本」を含む且つ lat/lng NULL
  const { rows: targets } = await client.query(`
    SELECT id, name, location FROM ${SCHEMA}.events
    WHERE location ILIKE '%日本%'
  ...
  for (let i = 0; i < targets.length; i++) {
    ...
    try {
      ...
      if (!result) {
        console.log(`  [${i + 1}/${targets.length}] SKIP (低精度): ${name?.slice(0, 50)}`)
        skipCount++
      } else {
        if (!DRY_RUN) {
          await client.query(
