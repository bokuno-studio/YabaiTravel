/**
 * 本番クロールスクリプト
 * CHECK_TARGET_URLS.md の全ソースからレース情報を取得し、
 * LLM（Claude Haiku）で詳細を補完して DB に投入
 *
 * フェーズ:
 *   1. 各ソースの一覧ページからレース URL を収集
 *   2. 各レースの詳細ページを取得
 *   3. LLM で構造化抽出
 *   4. DB に投入（重複スキップ、空フィールドのみ更新）
 *
 * 使い方:
...

{
  "event": {
    ...
    "location": "開催地（都市名, 国）",
    "country": "国名（日本語）",
    "race_type": "spartan|trail|hyrox|obstacle|adventure|marathon|triathlon|other のいずれか",
    "entry_url": "申込URL（あれば）",
    ...
    if (url.includes('devilscircuit.com')) {
      ...
      $('h2, h3').each((_, el) => {
        ...
        if (/^(Delhi|Mumbai|Bengaluru|Pune|Hyderabad|Kochi|Chennai|Guwahati|Jaipur|Lucknow|Indore|Ahmedabad|Dubai)/i.test(t)) {
          races.push({ name: `Devils Circuit ${t}`, official_url: url, entry_url: url, location: `${t}, India`, race_type: 'obstacle', source: 'devils-circuit' })
        }
      })
