/**
 * Time Outdoors — trail and marathon events
 */
export const SOURCE_NAME = 'time-outdoors'
export const SOURCE_URLS = ['https://www.timeoutdoors.com/']
...
export function parse(html, url, cheerioLoad, ctx) {
  ...
  $('a[href]').each((_, el) => {
    ...
    if (/\/(category|categories|tag|tags)/i.test(fullUrl)) return
