/**
 * Le Sportif — French marathon and trail events
 */
export const SOURCE_NAME = 'le-sportif'
export const SOURCE_URLS = ['https://www.le-sportif.com/']
...
export function parse(html, url, cheerioLoad, ctx) {
  ...
  $('a[href]').each((_, el) => {
    ...
    if (!/race|event|marathon|trail|course|run/i.test(href + ' ' + text)) return
    const fullUrl = href.startsWith('http') ? href : new URL(href, url).href
    if (/\/(results?|classement|palmares)/i.test(fullUrl)) return
