...
flowchart TB
    ...
    subgraph PASS1["パス1: 公式ページ取得 + LLM 抽出"]
        P1_FETCH[official_url を取得\n（なければ Tavily フォールバック）] --> P1_LLM[LLM で構造化抽出]
    end

    P1_LLM --> PASS0

    subgraph PASS0["パス0: race_type 再分類"]
        P0_CHECK{race_type = other?}
        P0_CHECK --> |Yes| P0_LLM[イベント名で専用 LLM 再分類]
        P0_LLM --> P0_RESULT[確定した race_type で上書き]
        P0_CHECK --> |No| P0_SKIP[そのまま使用]
    end

    P0_RESULT --> P1_WRITE[(events / categories 初回書き込み)]
    P0_SKIP --> P1_WRITE
    ...
    subgraph PASS3["パス3: Tavily 検索で補完"]
        P3_QUERY[空欄項目ごとに検索クエリを生成] --> P3_SEARCH[Tavily 検索]
...
| ブロック | 実行条件 | 処理内容 | LLM 呼び出し |
|---------|---------|---------|-------------|
| パス0 | race_type が `other` または未設定 | イベント名のみで race_type を専用プロンプトで再分類 | 1回（max_tokens: 32） |
| パス1 | 常時 | 公式ページ取得 → LLM で全フィールド抽出。official_url がない場合は Tavily 検索でフォールバック | 1〜3回 |
| パス2 | パス1 後に空欄あり | 同一ドメイン内の関連ページを探索 → LLM で追加抽出 | 最大5回 |
| パス3 | パス2 後に高優先度空欄あり | Tavily 検索で外部ソースから補完 | 項目数分 |
